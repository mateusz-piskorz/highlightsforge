...
const imgSchema = z.object({
    img: z
        ...
        .refine((file: File) => ACCEPTED_IMAGE_TYPES.includes(file.type), `Allowed file extensions: ${ACCEPTED_IMAGE_EXT.join(', ')}`)
        .nullable(),
});
</script>
<template>
    ...
    >
        <div :class="cn(btnState ? 'space-x-4 text-center' : 'h-full')">
            <label :class="cn('relative mx-auto', !btnState && 'flex h-full w-full flex-col items-center justify-center')">
                <Button v-if="btnState" variant="outline">Choose different</Button>
                <template v-else>
                ...
                <input
                    :class="cn('absolute top-0 left-0 h-full w-full cursor-pointer opacity-0')"
                    type="file"
                    accept="image/*"
                    @change="
                        async (event: any) => {
